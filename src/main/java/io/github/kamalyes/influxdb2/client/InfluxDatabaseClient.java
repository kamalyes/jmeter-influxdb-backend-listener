...
public class InfluxDatabaseClient {
     ...
     * Writes {@link List<Point>} when Batch size equals to size of the
     * {@link List<Point>}; cleans {@link List<Point>} after writing.
     *
    ...
    /**
     * Closes Influx DB client, cancels timers to write {@link Point}, cleaning
     * {@link List<Point>} collection; writes Points before closing.
     */
    ...
    /**
     * Imports {@link Point} from {@link List<Point>} collection if items exists.
     * Cleans {@link List<Point>} after writing.
     * Manages Error Amount counter. The limit of the errors occurred one by one is
     * 5 (the 5 - is default setting and can be updated in UI of the plugin - the
     * key is KEY_INFLUX_DB_THRESHOLD_ERROR). After 5 such errors, import is
     * stopping till and of the tes.
     * If less 5 error has occurred and next import attempt was successful - errors
     * counter will be refreshed.
     */
    public synchronized void importData() {
        ...
        if (!this.points.isEmpty() && this.errorsAmount.size() <= this.influxDBConfig.getInfluxdbThresholdError()) {
            try {
                ...
                if (!this.errorsAmount.isEmpty()) {
                    this.errorsAmount.clear();
                    this.LOGGER.warn("Counter of the errors refreshed since import was done successfully.");
                }

                this.LOGGER.info("Data has been imported successfully, batch with size is --> " + this.points.size()
                        + ", elapsed time is --> " + (end - start) + " ms");
                ...
                this.LOGGER.error("Error has occurred, batch with size " + this.points.size()
                        + " was not imported, see the details --> " + e.getMessage());
    ...
    public synchronized void setupInfluxClient() {
        ...
        try {
            this.influxDB = InfluxDBClientFactory.create(this.influxDBConfig.getInfluxDBUrl(),
