...
import io.github.kamalyes.config.InfluxDBConfig;
import io.github.kamalyes.config.TestStartEndMeasurement;
import io.github.kamalyes.config.VirtualUsersMeasurement;
import io.github.kamalyes.jmeter.samplers.SampleResultPointContext;
import io.github.kamalyes.jmeter.samplers.SampleResultPointProvider;
...
public class JmeterInfluxDBBackendListenerClient extends AbstractBackendListenerClient implements Runnable {
     ...
     * Logger.
     */
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(JmeterInfluxDBBackendListenerClient.class);
     ...
     * In a CI/CD automated performance test, a Jenkins or Bamboo build id would be
    ...
    public void handleSampleResults(List<SampleResult> sampleResults, BackendListenerContext context) {
        ...
        for (SampleResult sampleResult : allSampleResults) {
            ...
            // Determine the type of sample, whether it is a request or a transaction
            // controller
            ...
            if ((null != regexForSamplerList && sampleResult.getSampleLabel().matches(regexForSamplerList))
                ...
                sampleResultContext.setTimeToSet(
                        System.currentTimeMillis() * ONE_MS_IN_NANOSECONDS + this.getUniqueNumberForTheSamplerThread());
                sampleResultContext
    ...
    public Arguments getDefaultParameters() {
        ...
        arguments.addArgument(InfluxDBConfig.KEY_INFLUX_DB_FLUSH_INTERVAL,
                Integer.toString(InfluxDBConfig.DEFAULT_INFLUX_DB_FLUSH_INTERVAL));
        arguments.addArgument(InfluxDBConfig.KEY_INFLUX_DB_MAX_BATCH_SIZE,
                Integer.toString(InfluxDBConfig.DEFAULT_INFLUX_DB_MAX_BATCH_SIZE));
        arguments.addArgument(InfluxDBConfig.KEY_INFLUX_DB_THRESHOLD_ERROR,
                Integer.toString(InfluxDBConfig.DEFAULT_THRESHOLD_ERROR));
        ...
        arguments.addArgument(InfluxDBConfig.KEY_RESPONSE_BODY_LENGTH,
                Integer.toString(InfluxDBConfig.DEFAULT_RESPONSE_BODY_LENGTH));
    ...
    public void setupTest(BackendListenerContext context) {
        ...
        this.runId = context.getParameter(KEY_RUN_ID, "R001"); // Will be used to compare performance of R001, R002, etc
                                                               // of 'Test'
        ...
        Point setupPoint = Point.measurement(TestStartEndMeasurement.MEASUREMENT_NAME)
                .time(System.currentTimeMillis(), writePrecision)
    ...
    public void teardownTest(BackendListenerContext context) throws Exception {
        ...
        Point teardownPoint = Point.measurement(TestStartEndMeasurement.MEASUREMENT_NAME)
                .time(System.currentTimeMillis(), writePrecision)
        ...
        try {
            ...
            LOGGER.error(String.format("Error waiting for end of scheduler %s", e));
        }
    ...
    private void addVirtualUsersMetrics(int minActiveThreads, int meanActiveThreads, int maxActiveThreads,
        ...
        Point virtualUsersMetricsPoint = Point.measurement(VirtualUsersMeasurement.MEASUREMENT_NAME)
                .time(System.currentTimeMillis(), writePrecision)
    ...
    public void importDataByTimer(InfluxDBConfig conf, Logger logger) {
        ...
        this.timer.schedule(new TimerTask() {
            @Override
            public void run() {
                LOGGER.debug(String.format("Running the timer: %s" , LocalDate.now()));
