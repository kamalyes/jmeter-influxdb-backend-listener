...
import io.github.kamalyes.config.RequestMeasurement;
...
public class SampleResultPointProvider {
    ...
    /**
     * Gets {@link Point}, returns the OK or KO jmeter point, depends from the
     * sample result.
     * 
    ...
    public Point getPoint() {
        ...
        point.addTag(RequestMeasurement.Tags.RESULT, result);
        return point;
    }
     ...
     * @return the normalized string if parameter @param isToBeSaved set to true;
     *         'noData' string otherwise.
     */
     ...
     * 
     * @return first non null assertion failure message if assertionResults is not
     *         null, 'noData' string otherwise.
     */
    ...
    private String getErrorBody() {
        ...
        if (errorBody != null && !errorBody.isEmpty()) {
            return Arguments.getSubstring(Arguments.getEscapedString(errorBody),
    ...
    private Point getDefaultPoint() {
        ...
        return Point.measurement(RequestMeasurement.MEASUREMENT_NAME)
                .time(this.sampleResultContext.getTimeToSet(), WritePrecision.NS)
                .addTag(RequestMeasurement.Tags.REQUEST_NAME,
                        this.sampleResultContext.getSampleResult().getSampleLabel())
                ...
                .addTag(RequestMeasurement.Tags.RESULT_CODE,
                        this.sampleResultContext.getSampleResult().getResponseCode())
                ...
                .addTag(RequestMeasurement.Tags.ERROR_RESPONSE_BODY,
                        this.getErrorBodyToBeSaved(this.sampleResultContext.isErrorBodyToBeSaved()))
                .addField(RequestMeasurement.Fields.ERROR_COUNT,
                        this.sampleResultContext.getSampleResult().getErrorCount())
                .addField(RequestMeasurement.Fields.REQUEST_COUNT,
                        this.sampleResultContext.getSampleResult().getSampleCount())
                .addField(RequestMeasurement.Fields.RECEIVED_BYTES,
                        this.sampleResultContext.getSampleResult().getBytesAsLong())
                .addField(RequestMeasurement.Fields.SENT_BYTES,
                        this.sampleResultContext.getSampleResult().getSentBytes())
                ...
                .addField(RequestMeasurement.Fields.CONNECT_TIME,
                        this.sampleResultContext.getSampleResult().getConnectTime())
                .addField(RequestMeasurement.Fields.PROCESSING_TIME,
                        this.sampleResultContext.getSampleResult().getLatency()
                                - this.sampleResultContext.getSampleResult().getConnectTime());
